...
float px, py, pRaio;
int corPacman = color(255, 253, 56);
...
int[] corFantasmas = {color(66, 197, 244) /* azul */, color(244, 164, 66)/* laranja */, 
                      color(244, 75, 66)/* vermelho */, color(247, 173, 211)/* rosa */};
float[] vFantasmas = {1.7f, 1.5f, 1.95f, 2}; // velocidades dos fantasmas
int[][] mapaFantasmas;
...
public void draw(){
  ...
  if (!jogoIniciado) { // ecra do menu e highscores
    ...
    image(introPacImg, margemH*2, 0, 0.371875f*(height - 2*margemV), height - 2*margemV);
    ...
    try {
      ...
      if(fout.exists()){
        ...
        for (int i = 0; i < 4; i++) {
          for (int j = 0; j < 3; j++) {
            text(pontuacoes[i][j], margemH*13 + width*2/3, margemV*(i+6.25f) + (height - 5*margemV)*(i)/4 + 27*j); 
          }
        }
      }
    } catch (IOException e) {
...
public void comecarJogo(float dif) {
   ...
   for (int i = 0; i < fantasmas; i++) {
     ...
     float pFy = posFantasmas[i][1];
      ...
      int y = (int)Math.round((pFy + 0.5f - margemV/1.5f)/tamanho);
       ...
       if (posFantasmas[i][2] - 1 < 0.1f) { // para cima
         y = (int)Math.round((pFy + 0.5f - margemV*2)/tamanho) + 1;
       } else if (posFantasmas[i][2] - 2 < 0.1f) { // para baixo
         y = (int)Math.round((pFy + 0.5f)/tamanho);
       } else if (posFantasmas[i][2] - 3 < 0.1f) { // para a esquerda
         x = (int)Math.round((pFx + 0.5f - margemH*2)/tamanho) + 1;
       } else if (posFantasmas[i][2] - 4 < 0.1f) { // para a direita
     ...
     if (!obstaculo[i]) {
       ...
       if (dirObstaculoFantasma[i] == 0) { // o obstaculo esta a aparecer pela primeira vez
          if ((pacY != y)) {
            if (((posFantasmas[i][2] - 1 < 0.1f) && (posFantasmas[i][2] - 1 > -0.1f) && (mapaFantasmas[x][y-1] == 0)) || (pacY - y < 0)) { 
              // obstaculo esta acima do fantasma
              dirObstaculoFantasma[i] = 1; 
              contornarObstaculo(i, pacX, pacY, x, y, pFx, pFy);} // obs cima
            else if (((posFantasmas[i][2] - 2 < 0.1f) && (posFantasmas[i][2] - 2 > -0.1f) && (mapaFantasmas[x][y+1] == 0)) || (pacY - y > 0)) { 
              // obstaculo esta abaixo do fantasma
              dirObstaculoFantasma[i] = 2; 
              contornarObstaculo(i, pacX, pacY, x, y, pFx, pFy);} // obs baixo
          } else if (pacX != x) {
            if (((posFantasmas[i][2] - 3 < 0.1f) && (posFantasmas[i][2] - 3 > -0.1f) && (mapaFantasmas[x-1][y] == 0)) || (pacX - x <= 0)) { 
              // obstaculo esta a esquerda do fantasma
              dirObstaculoFantasma[i] = 3; 
              contornarObstaculo(i, pacX, pacY, x, y, pFx, pFy);
            } else if (((posFantasmas[i][2] - 4 < 0.1f) && (posFantasmas[i][2] - 4 > -0.1f) && (mapaFantasmas[x+1][y] == 0)) || (pacX - x > 0))  { 
...
public void desenharFantasmas() {
  ...
  for (int i = 0; i < fantasmas; i++) {
    if (!fantasmaComido[i]) {
      ...
      posFantasmas[i][1] -= 3.5f; // fazer o fantasma mais "alto" que o pacman
      
      // desenhar corpo do fantasma
      ellipse(posFantasmas[i][0], posFantasmas[i][1], pRaio, pRaio);
      rect(posFantasmas[i][0] - pRaio/2.0f, posFantasmas[i][1], pRaio, pRaio/1.5f);
      
      // desenhar cauda do fantasma, composta por 3 circulos
      ellipse(posFantasmas[i][0] - raioFant, posFantasmas[i][1] + pRaio/1.5f, pRaio/3.0f, pRaio/3.0f);
      ellipse(posFantasmas[i][0], posFantasmas[i][1] + pRaio/1.5f, pRaio/3.0f, pRaio/3.0f);
      ellipse(posFantasmas[i][0] + raioFant, posFantasmas[i][1] + pRaio/1.5f, pRaio/3.0f, pRaio/3.0f);
      
      
      // adicionar olhos ao fantasma
      fill (255);
      ellipse(posFantasmas[i][0] - pRaio/4.2f, posFantasmas[i][1], pRaio/3, pRaio/2.5f);
      ellipse(posFantasmas[i][0] + pRaio/4.2f, posFantasmas[i][1], pRaio/3, pRaio/2.5f);
      ...
      switch ((int)Math.round(posFantasmas[i][2])) {
        case 1: // fantasma a andar para cima
          ellipse(posFantasmas[i][0] - pRaio/4.2f, posFantasmas[i][1] - pRaio/8, tamanhoIris, tamanhoIris);
          ellipse(posFantasmas[i][0] + pRaio/4.2f, posFantasmas[i][1] - pRaio/8, tamanhoIris, tamanhoIris);
          break;
        ...
        case 3: // fantasma a andar para a esquerda
          ellipse(posFantasmas[i][0] - pRaio/4.2f - pRaio/10, posFantasmas[i][1], tamanhoIris, tamanhoIris);
          ellipse(posFantasmas[i][0] + pRaio/4.2f - pRaio/10, posFantasmas[i][1], tamanhoIris, tamanhoIris);
          break;
      ...
      posFantasmas[i][1] += 3.5f; // compensar o ajuste da altura do fantasma
    }
  }
}
